...
def GenerateMessageId(message, meaning=''):
  ...
  ########
  new_id = str(fp & 0x7fffffffffffffffL)
  id_exists = False
#  for line in msg_cvt_file:
#    if new_id in line:
#        id_exists = True
#        break
  msg_cvt_file_path = 'output/message_convert_result.xtb'
  if not os.path.isfile(msg_cvt_file_path):
    msg_cvt_file = open(msg_cvt_file_path, 'w')
    msg_cvt_file.write('')
    msg_cvt_file.close()
    
  if new_id in open(msg_cvt_file_path).read():
    id_exists = True
  if id_exists:
    print( "%s exists" % (new_id) )  
  if not id_exists:
    msg_cvt_file = file(msg_cvt_file_path, 'a')
    msg_cvt_file.writelines( '<translation id="%s">%s</translation>\n' % ( new_id,  escape(message) ) )
    msg_cvt_file.close()
  #######
  return new_id
